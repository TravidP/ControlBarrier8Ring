...
def load_batch_runs(group_name: str, batch_dir: Path) -> list:
    ...
    for _, row in summary_df.iterrows():
        ...
        available_cols = [
            c for c in [TIME_COL, DIST_COL, ENERGY_COL, SPEED_COL, THROUGHPUT_COL, DELAY_COL, SAFETY_COL]
            if c in df.columns
        ]
        ...
        datasets.append((df, run_label, group_name, n_vehicles, vehicle_csv))
...
def plot_speed_vehicle_band(ax: plt.Axes, dataset_item: tuple) -> None:
    _, run_label, group_name, vehicle_count, vehicle_csv = dataset_item
...
def plot_gap_all_runs_with_band(ax: plt.Axes, datasets: list) -> None:
    groups: dict[tuple[str, int], list] = {}
    for item in datasets:
        _, _, group_name, vehicle_count, vehicle_csv = item
    ...
    for i, (vc, tg) in enumerate(sorted(target_hlines.items())):
        ax.axhline(tg, color="#222222", linewidth=1.5,
...
def report_convergence_times(datasets: list) -> None:
    ...
    for item in datasets:
        _, _, _, vehicle_count, vehicle_csv = item
        if vehicle_csv is None or not Path(vehicle_csv).is_file():
            continue
        try:
            vdf = pd.read_csv(vehicle_csv).sort_values(TIME_COL)
...
# ------ Figures Setup --------
fig_throughput, ax_throughput = plt.subplots(figsize=(8, 5))
...
fig_gap_all, ax_gap_all = plt.subplots(figsize=(8, 5))
...
fig_speed_r1, ax_speed_r1 = plt.subplots(figsize=(8, 5))
...
fig_gap_r1, ax_gap_r1 = plt.subplots(figsize=(8, 5))
for item in datasets_run1:
...
fig_safety, ax_safety = plt.subplots(figsize=(8, 5))
plot_time_metric_with_band(
    ax_safety, datasets, SAFETY_COL,
    "",
    "Safety Violation Rate, r, [-]",
    ylim_bottom=0.0,
)
...
for n_val in unique_n_values:
    fig_stab_spd, ax_stab_spd = plt.subplots(figsize=(8, 5))
    fig_stab_gap, ax_stab_gap = plt.subplots(figsize=(8, 5))
