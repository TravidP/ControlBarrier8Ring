...
def run_simulation(args, run_index=1, run_output_dir=RESULTS_DIR):
    ...
    a_off     = 2.5    # comfort deceleration [m/s²] (wet pavement baseline)
    ...
    L_loop = 5.0    # physical loop length request loop[m]


    # CROW equations for all phase durations derived from kinematics
    v0_ms          = TARGET_SPEED_KMH / 3.6
    TL_YELLOW_TIME = math.ceil(t_r + v0_ms / (2.0 * a_off))                  # Eq A: t_gl [s]
    ...
    STOP_LINE_DIST         = 12.0       # Distance from conflict point to stop line [m]
    APPROACH_WINDOW_M      = 50.0       # Search window for traffic light candidates
    MAX_DECEL_MS2          = a_off      # Max comfortable deceleration [m/s²] 
    REACTION_TIME          = t_r        # Reaction time according to CROW [s]

    # Queue following parameters
    VEHICLE_LENGTH = 5.0                    # average of Tesla Model 3 and Audi e-tron lengths
    QUEUE_STOP_GAP = VEHICLE_LENGTH + 1.0   # Gap at which vehicles stop in a queue [m]
    QUEUE_SLOW_GAP = VEHICLE_LENGTH + 20.0  # Gap at which vehicles start slowing down in a queue [m]

    # Green-discharge: seconds after TL release during which vehicles may ignore the
    # closing-gap headway controller, allowing the queue to accelerate freely before
    ...
    LAP_CLEAR_RADIUS = 2.0     # Triggering space for starting new lap
    ...
    try:
        ...
        # Finding traffic lights within a radius from intersection
        ...
        # Determine which traffic light corresponds to which lane
        def _tl_yaw(tl):
            wps = tl.get_affected_lane_waypoints()
            return wps[0].transform.rotation.yaw if wps else 0.0

        # Assigning traffic light two groups corresponding to the two lanes
        ...
        # Mapping traffic light colors to trafficlights in CARLA
        _STATE_MAP = {
        ...
        # Freeze all lights to drive states based on PythonAPI 
        ...
        # States of trafficlight 
        phase              = 0               # 0 = Side 1 serving, 1 = Side 2 serving
        phase_state        = "fixed_green"   # States which phase traffic light is in
        phase_elapsed      = 0.0             # Counts time of active side
        gap_timer          = 0.0             # Counts gap at detection loop 
        extensions_granted = 0               # Counts how many extensions are granted using discharge loop

        # CROW Request  
        # Request_phase[0] is for direction 1, Request_phase[1] is for direction 2
        Request_phase = [False, False]

        # Color codes for traffic light in CARLA
        _GRN = "\033[30;42m"
        _YLW = "\033[30;43m"
        _RED = "\033[97;41m"
        _RST = "\033[0m"
       ...
       # Spawning with noise
        ...
        while True:
            ...
            #  CROW Request loop registration 
            for f in [0, 1]:
                # Only register a request if current phase is red
                if phase != f or phase_state == "yellow":
                    s_streep = stop_line_1 if f == 0 else stop_line_2

                    # Request loop between 0 and 10 m of the stop line
                    Request_loop_occupied = any(
                        (s_streep - 10.0) <= current_s_values[vid] <= s_streep
                        for vid in agent_ids
                    )

                    # Remember request if loop is occupied
                    if Request_loop_occupied:
                        Request_phase[f] = True

            # If phase is green, but request is registered for that phase, switch if queue is discharged or max green time is reached
            if phase_state in ["fixed_green", "ext_green"]:
                Request_phase[phase] = False

            # Initialization of parameters
            conflict_pair = None
            tl_speed_overrides = {} # Stores which vehicles are in speed overrides, is speed to 0
            tl_g1      = "---"      
            tl_g2      = "---"
            tl_g1_left = 0.0
            tl_g2_left = 0.0
            ...
            if step_count > WARMUP_STEPS:
                ...
                # CROW discharge loop occupancy check
                loop_far_edge  = stop_s - d     # Edge of loop at the start of loop
                loop_near_edge = stop_s - d + L_loop # Edge at end of loop
                loop_occupied  = any(
                    loop_far_edge <= current_s_values[vid] <= loop_near_edge
                    for vid in agent_ids
                )   # The loop is occuppied if a vehicles position is within the edges

                #  CROW Phase Altering
                if phase_state == "fixed_green":
                    # If phase is fixed green time is t_gg and no extensions can be granted
                    if phase_elapsed >= G_MAX:
                        print(f"[PHASE] phase={phase} reason=max-out(fixed) elapsed={phase_elapsed:.1f}s")
                        phase_state   = "yellow"
                        phase_elapsed = 0.0
                    elif phase_elapsed >= t_gg:
                        # Queue between loop has discharged and now extension can be given, switch to extended green
                        phase_state = "ext_green"
                        gap_timer   = 0.0

                elif phase_state == "ext_green":
                    # If discharge loop is occupied count time between arrivals of vehicles
                    if loop_occupied:
                        if gap_timer > 0.0:
                    ...
                    if phase_elapsed >= G_MAX:
                        terminate = True
                        reason    = "max-out"
                    elif gap_timer > t_H:       # Checking if arrival is outside threshold and gapping out
                        terminate = True
                        reason    = "gap-out"
                    ...
                    #  After yellow switch to other side
                ...
                # Map state to G1 / G2 CARLA signal strings
                ...
                # Approach-window speed overrides, braking for red or yellow light, onyl if within approach window
                ...
                # Checking for candidates which speed correction applies
                for group_cands, stop_line_s, tl_state in [
                    ...
                    for vid in group_cands:
                        ...
                        if tl_state == "yellow" and vid not in yellow_decisions:
                            v_ms = velocities[vid] / 3.6
                            stop_dist_needed = v_ms * REACTION_TIME + v_ms**2 / (2 * MAX_DECEL_MS2) if v_ms > 0 else 0.0
                            yellow_decisions[vid] = "stop" if stop_dist_needed < dist_to_stop else "go" # If to close to stop for safe stop accelerate

                        should_stop = tl_state == "red" or yellow_decisions.get(vid) == "stop" # Should stop if light is red or yellow and can stop
                        if should_stop:
            ...
            for i, vid in enumerate(sorted_agents):
                ...
                if step_count <= WARMUP_STEPS:
                    ...
                    # During discharge surpress negative headway for more realistic vehicle departure, phase is length of t_gg to allow all vehicles between loops to discharge 
                    ...
                    if vid in tl_speed_overrides:
                        target_speed = min(target_speed, tl_speed_overrides[vid])
                        # If queuing decelerate based on left distance
                    ...
                    target_speed = max(0.0, target_speed)
                    max_delta_kmh = COMFORTABLE_MAX_ACCEL_MS2 * DT * 3.6 # For clipping acceleration at max comfortable
